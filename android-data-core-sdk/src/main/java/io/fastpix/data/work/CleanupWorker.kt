        ...
        return try {
            ...
            if (SDKBuildConfig.SDK_URL.isBlank()) {
                ...
                if (!savedSdkUrl.isNullOrBlank()) {
                    ...
                    Logger.log("CleanupWorker", "Restored SDK_URL from SharedPreferences: $savedSdkUrl")
                } else {
                    Logger.logWarning("CleanupWorker", "SDK_URL not found in SharedPreferences, cannot proceed with cleanup")
            ...
            withContext(Dispatchers.IO) {
                if (persistedEvents.isNotEmpty()) {
                    Logger.log("CleanupWorker", "Found ${persistedEvents.size} persisted events to send")
                    
                    // Send persisted events
                    val success = eventDispatcher.sendPersistedEvents(persistedEvents)
                    
                    if (success) {
                        // Clear persisted events after successful send
                        persistenceManager.clearPendingEvents()
                        Logger.log("CleanupWorker", "Successfully sent ${persistedEvents.size} persisted events and cleared storage")
                    } else {
                        Logger.logWarning("CleanupWorker", "Failed to send some persisted events, keeping them for retry")
                    }
                } else {
